...
const getAppParamValue = (paramName, { defaultValue = undefined, removeFromUrl = false } = {}) => {
	...
	const storageKey = `automedia_${toSnakeCase(paramName)}`;
...
const getAppParams = () => {
	if (getAppParamValue("clear_access_token") === 'true') {
		...
		storage.removeItem('automedia_access_token');
	}
	return {
		...
		functionsVersion: getAppParamValue("functions_version", { defaultValue: import.meta.env.VITE_APP_FUNCTIONS_VERSION || import.meta.env.VITE_BASE44_FUNCTIONS_VERSION }),
